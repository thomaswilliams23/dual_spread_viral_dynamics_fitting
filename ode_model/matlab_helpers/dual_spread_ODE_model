function vec_out = dual_spread_ODE_model(t, Y, params)
    %the RHS of the dual spread TEIV model
    ...
    %indices will be helpful
    num_compartments = length(Y);
    
    assert(num_compartments == 2*latent_stages + 3*inf_stages + 6);
