function output = ABC_importance_sampler_with_clustering_stats(init_params_as_struct, ...
    params_to_fit, moi, sheet_size, sim_times, indices_of_obs_times, obs_inf_data, ...
    obs_clust_data, eps_err_val, eps_pur_val, num_particles, prior_funcs, prev_samples, prev_weights)
...
fluor_err_vec = zeros(1,num_particles);
clust_err_vec = zeros(1,num_particles);
...
%count number of clustering calculations where no infected cells were found
num_void_calc = 0;
...
while n_particles_found<num_particles
    ...
    fluor_err_this_batch = zeros(1,sims_this_batch);
    clust_err_this_batch = zeros(1,sims_this_batch);
    ...
    void_calc_this_batch = zeros(1,sims_this_batch);
    ...
    parfor sim_ind = 1:sims_this_batch
        ...
        if in_valid_region(sim_ind)
            ...
            %count void calculations
            void_calc_this_batch(sim_ind) = model_sim.tot_clustering_errs;


            %compute errors
            fluor_err_this_sample=data_to_sim_dist(obs_inf_data, model_pred_cumul_inf);
            clust_err_this_sample=data_to_sim_dist(obs_clust_data, model_pred_clust);



            %if errors both within thresholds
            if (fluor_err_this_sample<eps_err_val) && (clust_err_this_sample<eps_pur_val)

                %sample was accepted
                samples_are_valid(sim_ind)=1;

                %save fluorescence error and ppc
                fluor_err_this_batch(sim_ind)=fluor_err_this_sample;

                %save clustering stat error
                clust_err_this_batch(sim_ind) = clust_err_this_sample;

                %save the full fluorescent cell time series and true Pcc
                %for this sim
    ...
    %update count of void calculations
    num_void_calc = num_void_calc + sum(void_calc_this_batch((samples_are_valid>0)));
    ...
    valid_fluor_err_this_batch = fluor_err_this_batch((samples_are_valid>0));
    valid_clust_err_this_batch = clust_err_this_batch((samples_are_valid>0));
    ...
    if tot_valid_particles<num_particles
        ...
        fluor_err_vec((n_particles_found+1):tot_valid_particles) = valid_fluor_err_this_batch;
        clust_err_vec((n_particles_found+1):tot_valid_particles) = valid_clust_err_this_batch;
        ...
        fluor_err_vec((n_particles_found+1):num_particles) = ...
            valid_fluor_err_this_batch(:,1:(num_particles-n_particles_found));
        clust_err_vec((n_particles_found+1):num_particles) = ...
            valid_clust_err_this_batch(:,1:(num_particles-n_particles_found));
