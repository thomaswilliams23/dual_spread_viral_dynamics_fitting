function beta_val = infer_beta_on_peak_time_contour(...
    lookup_table_name, alpha_range, beta_range, alpha_val, growth_rate)
...
[~,beta_0_ind] = min(abs(constructed_beta_vals - growth_rate));
beta_0 = beta_range(beta_0_ind);
solve_opts = optimset('display', 'off');
beta_val = fsolve(@(x) ppval(beta_poly, x) - growth_rate, beta_0, solve_opts);
