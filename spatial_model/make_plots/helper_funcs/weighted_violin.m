...
%            ksdensity()
% weights:   weights associated with each of the samples [ADDED BY ME]
%
% OUTPUT
%
% h:     figure handle
...
%}
%%
function[h,L,MX,MED,bw]=weighted_violin(Y,varargin)
...
if isempty(find(strcmp(varargin,'weights')))==0
    weights = varargin{find(strcmp(varargin,'weights'))+1};
    uses_weights = 1;
end
%%
if size(fc,1)==1
...
for i=1:size(Y,2)
    ...
    if uses_weights
        if isempty(b)==0
            [f, u, bb]=ksdensity(Y{i},'bandwidth',b(i), 'Weights', weights(:,i));
        elseif isempty(b)
            [f, u, bb]=ksdensity(Y{i}, 'Weights', weights(:,i));
        end
    else
        if isempty(b)==0
